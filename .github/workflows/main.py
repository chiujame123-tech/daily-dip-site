    ...
    for i in range(retries):
        try:
            ...
            if resp.status_code == 200:
                ...
                # 這裡會印出具體錯誤原因 (例如：Upgrade Required 或 Date range error)
                log_msg(f"❌ API Error: {resp.status_code} | {resp.text[:200]}")
                return None
...
def get_polygon_data(ticker, multiplier=1, timespan='day'):
    ...
    # 🌟 關鍵修正：將結束日期改為「昨天」，避開 Starter 方案的盤中限制
    yesterday = datetime.now() - timedelta(days=1)
    end_date = yesterday.strftime('%Y-%m-%d')
    start_date = (yesterday - timedelta(days=250)).strftime('%Y-%m-%d')
    ...
    if data and data.get('status') == 'OK' and data.get('results'):
        ...
        # 如果還是沒有數據，這次我們會知道原因
        if data:
            log_msg(f"⚠️ No Data for {ticker}: Status={data.get('status')} | Count={data.get('count')}")
        else:
            log_msg(f"⚠️ No Data response for {ticker}")
        return None
...
# --- 4. 主程式 ---
def main():
    print("🚀 Starting Polygon Pro Analysis (Starter Plan Fix)...")
    ...
    # 錯誤日誌顯示 (如果有)
    error_html = "<br>".join(DEBUG_LOG[-5:]) if DEBUG_LOG else "" 
